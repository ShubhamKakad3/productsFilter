...
import EmptyState from "@/components/products/EmptyState";
 // these are the options for the dropdown menu
  ...
  // as const use to make it constant and readonly can not change or redefine
const COLOR_FILTERS = {
  id: 'color',
  name: 'Color',
  options:[
...
const PRICE_FILTERS = {
  ...
  options: [
    ...
    //  custom price options in jsx

  ]}as const 

const SUBCATEGORIES = [
  { name: "T-shirt", selected: true, href: "#" },
...
export default function Home() {
  ...
  const applyArrayFilter = ({
    ...
    // checking for the element exist or not in the Array
    const isFilterApplied = filter[category].includes(value as never)
    // if element is exist
    if (isFilterApplied) {
      setFilter((prev) => ({
        ...prev,
        [category]:prev[category].filter((v)=> v !== value)      // important logic
      }))
    } else {
      setFilter((prev) => ({
       ...prev,
  ...
  // this is really amazing advance fetching  
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              {SORT_OPTIONS.map((option) => (
                // in setFilter keep all the previous options as it is only change for the sort or just overwright it
                <button
